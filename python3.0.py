...
def delete():
    ...
    with open('todo_list.json', 'r', encoding='cp1251') as json_file:
            contents = json.load(json_file)
    for i in contents:
        if i!=reversed(listbox_delete.curselection()):
            writer(i)
...
def delete_task():
    ...
    listbox_delete = Listbox(frame2, selectmode=SINGLE, width=50, highlightcolor='white',bg='#E0E0E0', selectbackground='#2F2F2F' )
